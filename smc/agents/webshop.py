...
class WebshopAgent(Agent):
    """
    Agent that interacts with the Webshop environment using a language model.
    """
    ...
    def _get_synced_client(self):
        ...
        new_client = WebshopEnvClient(**self.client_args)
        new_client.reset(idx=self.task_idx)
        for action in self.action_history:
            if self.action_format == "reflact":
                new_client.step("Reflection:\nskip\n\nAction:"+action)  
            else:
                new_client.step("Thought:\nskip\n\nAction:"+action)
        return new_client

    def _copy_state_to(self, new_agent: "WebshopAgent"):
        """Copies mutable state variables to the new agent."""
        new_agent.conversation = copy.deepcopy(self.conversation)
        new_agent.current_reward = self.current_reward
