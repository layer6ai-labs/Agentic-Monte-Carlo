...
from .utils import ItemTag, ItemTagWithCount, ItemTagWithCraftableTag, Recipe, ActionFailed, item_id_to_str
...
class CraftingTree:
    ...
    def get_max_reward(self, item_name: str, count: int = 1) -> int:
        surplus = {}  # Tracks leftovers to avoid double counting

        def calculate_score(name, needed):
            # use surplus first
            available = surplus.get(name, 0)
            taken_from_surplus = min(needed, available)
            
            needed -= taken_from_surplus
            surplus[name] = available - taken_from_surplus
            ...
            actual_item = name
            if self.is_tag(name):
                possible_items = list(self.get_items_with_tags(name))
                if possible_items:
                    actual_item = min(possible_items, key=lambda x: self.get_min_depth(x))

            # 3. Check Recipe
            recipes = self.itemid_recipes.get(actual_item) or self.tag_recipes.get(actual_item)
            
            if recipes:
                # crafting is double the sum of ingredients so that
                # the normalized score is 50% crafting, 50% gathering
                
                recipe = min(recipes, key=lambda r: self.get_min_depth_recipes([r]))
                output_qty = recipe.output_item.count
                
                num_crafts = ceil(needed / output_qty)
                total_produced = num_crafts * output_qty
                
                surplus[actual_item] = surplus.get(actual_item, 0) + (total_produced - needed)
                
                # recursively go through and calculate scores
                ingredients_score_sum = 0
                for ingredient in recipe.input_items:
                    ingredients_score_sum += calculate_score(
                        ingredient.item_tag.name, 
                        num_crafts * ingredient.count
                    )
                
                return ingredients_score_sum * 2
            
            else:
                # non-craftable item, return 1
                return needed * 1

        return calculate_score(item_name, count)

def main():
    tree = CraftingTree(minecraft_dir="agentenv_textcraft/")
    
    # Mirror the environment's sorting
    item_depth_list = list(tree.item_recipes_min_depth(1))
    sorted_list = sorted(item_depth_list, key=lambda x: x[1])

    for item_id, depth in sorted_list:
        print(item_id, depth, sep="\t")
