...
class APIAgent:
    def __init__(
        ...
        # ----------------------
        # self.logger = logging.getLogger(f"APIAgent-{model}")
        # self.logger.setLevel(logging.INFO)
        # if not self.logger.handlers:
        #     ch = logging.StreamHandler()  # prints to stdout
        ...
        while True:
            try:
                if "gpt-5" in self.model:
                    ...
                    ## with reasoning_effort control testing
                    # response = self.client.responses.create(
                    #     model=self.model,  # e.g., "gpt-5.1"
                    #     input=[
                    #         {
                    #             "role": c["role"],
                    #             "content": c["content"],
                ...
                # self.logger.info(
                #     f"[USAGE] model={self.model} "
                #     f"in={getattr(usage, 'prompt_tokens', None)} "
                #     f"out={getattr(usage, 'completion_tokens', None)} "
                #     f"reasoning={getattr(usage, 'reasoning_tokens', None)} "
