        ...
        while not done:
            ...
            if isinstance(agent, Agent):
                env_message = ConversationMessage(
                    {"from": "human", "loss": None, "value": state, "reward": reward}
                )
                ...
                conversation_tokenized["input_ids"] += env_message_tokenized[
                    "input_ids"
                ]
                ...
                conversation.append(
                    APIConversationMessage(
                        {"role": "user", "content": state, "reasoning_content": None, "reward": reward}
                    )
                )
            else:
                raise NotImplementedError
