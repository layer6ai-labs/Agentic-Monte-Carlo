...
class BaseAdapter:
    ...
    @staticmethod
    def parse_reflact(text: str) -> ActionWithReflection:
        """
        ReflAct format:
        ```
        Reflection:
        ...
        _split = text.rsplit("Action:", 1)
        if len(_split) == 0:
            _reflection, _action = text
            invalid_format_flg = True
        elif len(_split) == 1:
            _reflection, _action = _split[0], ""
            invalid_format_flg = True
        else:
            assert len(_split) == 2
            _reflection, _action = _split
        
        reflection = _reflection.split("Reflection:")
        if len(reflection) == 1:
            reflection = reflection[0]
            invalid_format_flg = True
        else:
            reflection = reflection[1].strip()
        action = _action.strip()

        if invalid_format_flg:
            print("###RAW TEXT:\n", text)
        return ActionWithReflection(reflection, action)
    ...
    @staticmethod
    def to_reflact(action_with_reflection: ActionWithReflection) -> str:
        return f"Reflection:\n{action_with_reflection.reflection}\n\nAction:\n{action_with_reflection.action}"
    ...
    def action_parser(cls, action: str, action_format: ActionFormat) -> str:
        ...
        elif action_format == ActionFormat.REFLACT:
            return cls.parse_reflact(action).action
...
class Evaluator(BaseAgentEnvController):
    def eval(
        ...
        return EvaluationOutput(
            experiences=exps,
            score=rewards.mean(),
            success=np.logical_or(rewards == 1, rewards == 100).mean(),
            rewards=rewards,
            success_rate=np.logical_or(rewards == 1, rewards == 100).astype(int),
        )
...
class BaseTrainer(BaseAgentEnvController):
    ...
    def eval(
        ...
        return EvaluationOutput(
            experiences=exps,
            score=rewards.mean(),
            success=(rewards == 1 or rewards == 100).mean(),
        )
